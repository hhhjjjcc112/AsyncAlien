...
用户程序可以直接调用其中的函数（如 `__vdso_clock_gettime`），避免频繁陷入内核 syscall。

常见做法是同时映射两块区域：
- `vVAR`：共享数据区（通常 UR）
- `vDSO`：共享代码区（通常 URX）

用户态通过 auxv 的 `AT_SYSINFO_EHDR` 找到 vDSO ELF 基址。

## 2. 模板里两个 crate 的作用

`reference/vdso_crate_template` 里核心是两个 crate：
...
- AsyncModules 模板：`reference/vdso_crate_template`
