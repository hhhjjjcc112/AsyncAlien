...
use page_table::{MappingFlags, PageSize, PagingError, PagingIf, PagingResult, X64PTE, X64PageTable};
#[cfg(not(target_arch = "x86_64"))]
use page_table::{MappingFlags, PageSize, PagingError, PagingIf, PagingResult, Rv64PTE, Sv39PageTable};

use crate::{VmArea, VmAreaEqual, VmAreaType, VmIo};

// Architecture-specific type aliases
#[cfg(target_arch = "x86_64")]
...
impl<T: PagingIf<ArchPTE> + Debug> Debug for VmSpace<T> {
...
impl<T: PagingIf<ArchPTE>> VmSpace<T> {
    pub fn new() -> Self {
        Self {
            table: ArchPageTable::try_new().unwrap(),
            areas: BTreeMap::new(),
        }
    }
    ...
    pub fn area_iter(&self) -> Values<'_, usize, VmAreaType> {
        self.areas.values()
    }
}

impl<T: PagingIf<ArchPTE>> VmIo for VmSpace<T> {
