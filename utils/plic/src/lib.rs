...
/// The PLIC IO trait.
///
/// This trait is used to read and write the PLIC registers.
/// User should implement this trait to use the PLIC driver.
pub trait PlicIO: Debug + Send + Sync {
    /// Read a 32-bit word from the PLIC region at the given offset.
    fn read_at(&self, offset: usize) -> u32;
    /// Write a 32-bit word to the PLIC region at the given offset.
...
impl<const H: usize> PLIC<H> {
    ...
    pub fn enable(&self, hart: u32, mode: Mode, irq: u32) {
        ...
        let old_value = self.region.read_at(offset);
        self.region.write_at(offset, old_value | (1 << bit));
    }
    ...
    pub fn pending(&self, irq: u32) -> bool {
        ...
        let val = self.region.read_at(offset);
        (val & (1 << bit)) != 0
    }
    ...
    pub fn set_priority(&self, irq: u32, priority: u32) {
        ...
        self.region.write_at(offset, pri);
    }
    ...
    pub fn set_threshold(&self, hart: u32, mode: Mode, threshold: u32) {
        ...
        self.region.write_at(offset, threshold);
    }
    ...
    pub fn complete(&self, hart: u32, mode: Mode, irq: u32) {
        ...
        let contexts = self.context_index(hart, mode);
        let offset = CLAIM_COMPLETE_OFFSET + contexts * 0x1000;
        self.region.write_at(offset, irq)
    }
