...
fn read_resource_bytes(ctx: &AmlContext, value: &AmlValue) -> Option<Vec<ParsedResource>> {
    ...
    let mut parsed: Vec<ParsedResource> = Vec::new();
    ...
    while index < bytes.len() {
        ...
        match descriptor_type {
            ...
            0x08 if length >= 7 => {
                ...
                let base =
                    u16::from_le_bytes([bytes[payload_start + 1], bytes[payload_start + 2]])
                        as usize;
            ...
            0x09 if length >= 3 => {
                ...
                let base = u16::from_le_bytes([bytes[payload_start], bytes[payload_start + 1]])
                    as usize;
