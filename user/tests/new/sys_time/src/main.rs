...
use Mstd::{println, time::{clock_gettime_raw, sleep}};
...
fn main() -> i32 {
    ...
    assert_eq!(clock_gettime_raw(CLOCK_REALTIME, &mut realtime), 0);
    assert_eq!(clock_gettime_raw(CLOCK_MONOTONIC, &mut monotonic), 0);
    ...
    for _ in 0..4 {
        ...
        assert_eq!(clock_gettime_raw(CLOCK_MONOTONIC, &mut current), 0);
