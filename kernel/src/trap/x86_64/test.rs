...
pub fn record_breakpoint(rip: usize) {
    ...
    println!("[trap_test] hit breakpoint at RIP={:#x}", rip);
}

pub fn run() {
    BREAKPOINT_HIT.store(false, Ordering::Release);
    println!("[trap_test] trigger breakpoint");
    unsafe {
        asm!("int3", options(nomem, nostack));
    }
    assert!(
        BREAKPOINT_HIT.load(Ordering::Acquire),
        "trap_test: breakpoint handler was not reached"
    );
    println!("[trap_test] breakpoint self-test passed");
}